...
urlsOne = [f"http://10.244.0.158:8000/ms/{func}" for func in funcsOne]

funcsTwo = ["store_review_service","upload_user_review","upload_movie_review_handle"]
urlsTwo = [f"http://10.244.0.158:8000/ms/{func}" for func in funcsTwo]
...
def workflow():
    try:
        ...
        start = time.time()
        compose_res = requests.post("http://10.244.0.158:8000/ms/compose_review", data=json.dumps(review), headers=headers).text

        compose_res = json.loads(json.loads(compose_res))
        print(compose_res)
        print(type(compose_res))
        parallel_res = []
        threads = []
        for url in urlsOne:
        ...
        mr_res = requests.post("http://10.244.0.158:8000/ms/mr_compose_and_upload", data=json.dumps(result),
